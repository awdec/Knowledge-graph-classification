...
class Spider():
    ...
    def get_file_from_url(self, paper_link, save_path):
        ...
        try:
            path = "C:/Users/11205/Desktop/知识图谱/geckodriver.exe"
...
class NeurlPS_spider(Spider):
    ...
    def get_pdf_list_file(self):
        ...
        try:
            cond = EC.presence_of_element_located((By.CSS_SELECTOR, '#accepted-papers ul li'))
            WebDriverWait(driver, 30).until(cond)

            while True:
                elems = driver.find_elements(By.CSS_SELECTOR, '#accepted-papers ul li')

                print(elems)

                for elem in elems:
                    title_ele = elem.find_element(By.CSS_SELECTOR, 'h4 > a:nth-child(1)')
                    paper_link = elem.find_element(By.CSS_SELECTOR, 'h4 > a:nth-child(2)').get_attribute('href')
                ...
                try:
                    next_btn = driver.find_element(By.XPATH, '//nav//a[contains(text(),"Next")]')
        ...
        with open(self.target_file_name, "r", encoding='utf-8') as fpdf:
            for line in fpdf.readlines():
                if line == "\n":
                    if title and paper:
                        paper_cnt += 1
                        index.write(f"{paper_cnt} {title}\n")
                        self.url_list.append((title, paper))
                    title = None
                    paper = None
